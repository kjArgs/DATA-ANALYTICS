# SQL Practice Database: Startups

## Overview

This guide shows you how to recreate the **Startups** dataset as a MySQL database and practice SQL queries using it.
...
The original dataset contains 72 startup records. Missing values are represented by `NULL`. 
...
- MySQL Server
- MySQL Workbench, phpMyAdmin, or another MySQL client
- Basic knowledge of `CREATE`, `INSERT`, and `SELECT`

---

# 2. Create the Database

First, create a database named `startup_db`.
...
You can confirm the selected database with:
...
|---|---|---|
| `id` | `INT` | Unique identifier generated automatically |
| `name` | `VARCHAR(100)` | Name of the startup/company |
| `location` | `VARCHAR(100)` | Company's location |
| `category` | `VARCHAR(100)` | Industry/category |
| `employees` | `INT` | Number of employees |
| `raised` | `BIGINT` | Amount of money raised |
| `valuation` | `BIGINT` | Company valuation |
| `founded` | `INT` | Year the company was founded |
| `stage` | `VARCHAR(50)` | Funding/company stage |
| `ceo` | `VARCHAR(100)` | CEO name |
| `info` | `TEXT` | Additional company information |

---

# 4. Insert the Dataset

Run the following query after creating the table.
...
('Townie666', 'Brooklyn', NULL, 2, 20000, 80000, 2017, 'Seed', 'Sonny Li', '**'),
